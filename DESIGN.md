...
- An archived KOTH night, one whose event read answers `archived`, is drawn by `HistoricalBoard` instead: one card per source bracket with its literal name, the reported king as `mdi-crown` over the name in bold, then every BO1 in play order, numbered, as a two-side box built from the `SeriesBox` side and bar. The winner wears the `win` bar, bold, and a crown at the end of the side: `mdi-crown` in `primary-text` for a result the source records, `mdi-crown-outline` in muted ink for one inferred from the play order. The loser wears the `loss` bar. A series with no result wears `draw` on both sides and no crown; "Forfeit" in muted text where the order reads one, and an `mdi-information-outline` whose tooltip names the review reason. No score, because a best of one has none. An archived name is the written name alone, with no flag, race or link, until an account claims it. One legend over the brackets names the three bars and the two crowns.
