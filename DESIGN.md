...
- The series action bar lets any rostered member of a team that fields a side act, not the captains alone, because the API gate `acts_for_side` does (`next/src/helpers/series-actions.mjs`).
- An admin who plays no side reports through the player route, which answers 403 until the API takes an admin there; his schedule takes the admin route and lands (`next/src/components/player/ScheduleDialog.tsx`).
- The round cards on a player page name the opponent with his race alone. `GET /player-series` answers a reduced series row that carries no rating, so no MMR draws beside the name.
