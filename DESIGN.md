...
- A "Hide results" switch over the draw blanks every score and win mark and hides the standings, so a reader can watch the games first. It is the viewer's own choice, stored in `localStorage` under `hideResults`, and it starts off. `SeriesBox` and `StageView` read it through the `HIDE_RESULTS` provide key, so a page with no switch shows every result.
