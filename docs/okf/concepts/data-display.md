...
- Colour never carries a result alone. Text wears a text token, with one exception: a result seen from one side puts that side's score first and draws the score itself in `win`, `loss` or `draw`, so the order of the score is the second channel. A result wears no alert icon and no "You won" or "You lost". An alert icon is for a fault or a call to action.
