...
* **Update**: the KOTH run page's bracket bounds save on every night that is not closed, also while a series plays.
...
* **Update**: the KOTH nights list reads one page of 25 nights at a time and pages with `X-Total-Count`; an events list read with `limit` and `offset` stays edge cached for a non-admin.
