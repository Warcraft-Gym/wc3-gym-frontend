...
- Basics: the league, the name, the kind, "Part of" (an event of the same league that this one feeds, for a qualifier), the region, the start and end date, the start time, the round end zone, the description, a page link and a stream link. The round end zone is an IANA name from the browser's own list; a round of an event that names none ends at midnight where the reader is.
