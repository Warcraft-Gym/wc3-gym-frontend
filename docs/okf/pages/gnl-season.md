...
tags: [pages, events]
generated: { by: claude-code/claude-fable-5-1, at: 2026-09-20T01:10:00Z }
sources:
  - id: seasons
    resource: ../../../next/src/app/(app)/seasons/SeasonsView.tsx
...
**Seasons (`/seasons`).** One row per season: name, rounds, pick and ban order, series per fixture, and the phase (open, commenced, overdue, complete), with a warning mark on an overdue season and a chip that counts the series with no result, which opens the season with that list. A row opens the season. The admin dialog, for a new season or an edit, takes the name, the number of rounds, the pick and ban order, the series per fixture, the score system (standard or helpstone), the Discord role id, the start and end date, the map pool, the signups switch, the availability tools switch, the round end zone, the recent games floor with how many of the newest W3C seasons it counts over, the check-in switch, the largest MMR difference of each captain draft stage, and the fantasy grind pick. The round end zone is an IANA name; a round of a season that names none ends at midnight where the reader is. The check-in switch holds the check-in days (blank keeps check-in open all season) and the early check-in switch, which the dialog shows only while check-in is on; off, the round check-in asks nobody. Early check-in lets a player check in for any round that has not ended, and a new season starts with check-in and early check-in on and with a floor of 20 games over the last 2 W3C seasons. The dialog writes `PUT /events/{id}`; an edit reads the season again with `GET /events/{id}`, because the list read carries no stage, and writes `PUT /events/{id}/stages` only when a largest MMR difference was typed. That write replaces every field of every stage, so each stage goes back as it was read. A largest MMR difference is 1 or more and belongs to a captain draft stage alone; a blank one reads as the default of 100. A games count is over one W3C season or more, and a blank one counts every synced season. The row menu also exports the season as a spreadsheet and deletes it. A panel imports a season from a spreadsheet, by season name or id.
