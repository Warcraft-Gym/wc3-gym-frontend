...
function Side({ team, own, other }: { team: Row; own?: number; other?: number }) {
  return (
    <div className="flex min-w-0 basis-5/12 flex-col items-center gap-2">
      <h2 className="max-w-full text-2xl tracking-wide text-on-band [text-shadow:2px_2px_4px_rgba(var(--v-theme-band),0.8)] min-[960px]:text-4xl">
        {/* the banner stacks the logo over the name, so a name that wraps stays centred over its score */}
        <TeamName team={team} className="max-w-full flex-col gap-1 [&_.name]:break-words [&_.name]:whitespace-normal" />
      </h2>
