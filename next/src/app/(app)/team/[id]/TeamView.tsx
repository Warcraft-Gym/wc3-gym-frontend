...
export function TeamView({ id }: { id: string }) {
  ...
  const stats = [{ label: "Rank", value: rank ? `${rank.rank} of ${rank.of}` : "—" }, { label: "Series", value: recordFigure(record.wins, record.losses) ?? "—" }, { label: "Points", value: info?.final_score ?? "—" }, { label: "Points against", value: info?.points_against ?? "—" }, { label: "Points available", value: info?.points_available ?? "—" }];
