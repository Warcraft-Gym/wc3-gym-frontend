...
const EDGE_CACHED = /(\/events\/\d+\/ladder(\/players)?|\/events\/\d+\/(entrants|achievements)|\/events\/\d+\/stages\/\d+\/(series|standings)|\/home\/series|\/koth\/(nights\/\d+\/)?board|\/leagues|\/maps|\/config\/w3c|\/config\/settings\/\w+|\/users\/\d+\/ladder(\?season_id=\d+)?|\/users\/\d+\/history|\/(events|leagues)\/\d+\/teams(\/basic|\/\d+)?)$/;
