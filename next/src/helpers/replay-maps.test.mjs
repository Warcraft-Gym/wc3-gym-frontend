...
import { mapMismatch, mapMismatches, reportWarning, swapMapFields } from './replay-maps.mjs';
...
test('a swapped replay takes the map field it wrote with it', () => {
  // game 1 holds the map its own replay named, game 2 none: the map follows the file
  assert.deepEqual(swapMapFields({ 1: 8 }, 1, 2, (game) => (game === 1 ? 8 : null)), { 2: 8 });
  // both games hold the map of their own replay: the two fields swap with the two files
  assert.deepEqual(swapMapFields({ 1: 8, 2: 7 }, 1, 2, (game) => (game === 1 ? 8 : 7)), { 1: 7, 2: 8 });
  // a map the reporter named himself stays with its game
  assert.deepEqual(swapMapFields({ 1: 7 }, 1, 2, () => 8), { 1: 7 });
  assert.deepEqual(swapMapFields({}, 1, 2, () => null), {});
});
